...
Since the weight function generally depends on a \textit{global} parameter $R$ (ex. volume average of $\phi^\dagger\phi$), parallelization of multicanonical algorithms is not straightforward. We use the following method, adapted from \cite{Kajantie:1995kf}: 
...
For the "easy" order parameter $\phi^\dagger\phi$, the acceptance rate of the multicanonical update is generally good, $>90\%$. This is in agreement with rates reported in \cite{Kajantie:1995kf,Laine:1998qk}. 

\textbf{To do:} optimize the algorithm for more complicated order parameters. It should be safe to do perform the multicanonical step in parts, so that if we choose hopping term as the order parameter, we can first update half of the links, do global acc/rej, then update half of the Higgs and do acc/rej again etc.
...
\begin{itemize}
	...
	\item The Landau-Wang method. We use a variation of this, basically copied from old lecture notes by Kari Rummukainen. 
...
By default, the code assumes that all physical input parameters and fields are in natural lattice units, i.e. scaled dimensionless by the lattice spacing $a$. Here we describe the procedure for converting the continuum parameters into input parameters for the code for some implemented models. All examples here are for 3D theories.
...
\end{align}
This can be checked by comparing hopping terms in the two parametrizations as the ordering does make a difference. Note that $\frac12 \Tr \he\Phi\Phi = \he\phi\phi = \frac12 \phi_\mu \phi_\mu$.
...
Simulations in the electroweak $\gr{SU(2) \times U(1)}$ effective theory in 3d were discussed in \cite{Kajantie:1996qd}. Including the $\gr{U(1)}$ gauge field to the actions above is straightforward, but note that non-perturbatively there is a difference between ``compact'' (gauge group really is $\gr{U(1)}$) and ``non-compact'' (gauge group is real numbers under addition) formulations.

The link variable is just a complex number $u_i(x) = \exp(i \alpha_i(x))$; the simulation will operate on the real numbers $\alpha_i(x)$ (in code: u1field[x][dir]). The plaquette trace for a ``fundamental'' $\gr{U(1)}$ Wilson line is as 
\begin{align}
p_{ij}(x) = \exp\{i[\alpha_i(x) + \alpha_j(x+i) - \alpha_i(x+j) - \alpha_j(x)]\} \equiv \exp[i f_{ij}(x)].
\end{align}
As in \cite{Kajantie:1996qd} (and in the book by Jan Smit) we can consider actions where the plaquette is in an arbitrary irreducible representations of $\gr{U(1)}$. The reps can be labeled by an integer number $r$. A possible action is then
\begin{align}
S_\gr{U(1)} = \beta_G' \sum_{x, i<j} \Big[ 1 - \frac12 \Big(p_{ij}^r + (p_{ij}^*)^r \Big)\Big] = \beta_G' \sum_{x, i<j} \Big[ 1 - \cos (r f_{ij} )\Big].
\end{align}
Taking $\beta_G' \propto 1/r^2$, any $r$ gives the same naive continuum limit. In compact formulation, $r$ needs to be an integer and the gauge transformations operate on the $a_i(x)$ modulo $(2\pi)$, \textit{i.e.} in the simulations too we can restrict $-\pi < a_i(x) \leq \pi$. Formally $r \rightarrow 0$ corresponds to the non-compact formulation. 


In the compact theory, matter fields need to be in representations of $\gr{U(1)}$ and thus only integer charges are allowed. Of course, in continuum theory the charges can be normalized at will by rescaling the gauge coupling, in particular the Higgs covariant derivative in usual normalization is 
\begin{align}
D_i \phi = \partial_i \phi + \frac12 i g A^a_i \sigma_a + Y i g' B_i 
\end{align}
with $Y = 1/2$. The Higgs doublet $\Phi$, in matrix parametrization, transforms under a $\gr{SU(2) \times U(1)}$ gauge transformation as 
\begin{align}
...
It is best to rescale the charge, $\tilde{g}' \equiv Y g'$, and write the lattice action in normalization where Higgs is in $r=1$ irrep. Writing $u_i(x) = e^{i a \tilde{g}' B_i(x)}$, we find the correct continuum limit for the gauge action if 
\begin{align}
\beta_G' = \frac{1}{a \tilde{g}'^2 r^2}.
\end{align}
Note that our renormalization scheme and DR gives $g'$ instead of $\tilde{g}'$, but the conversion is trivial.

With the lattice gauge coupling defined as above, the lattice covariant derivative term is 
\begin{align}
...
\end{align}
with $\sigma_0$ again being the $2\times 2$ identity matrix. 
...
As a side note, we point out that the global radial update for the Higgs advertised in \cite{Kajantie:1995kf} is most likely not worth implementing because it scales very badly with volume, and it does not seem to be included in more recent works of the authors.
...
\end{align} 
Note that the $\gr{U(1)}$ field does contribute to the hopping term. In code it's useful to write $e^{-i \alpha_j(x) \sigma_3} = \cos \alpha_j(x) \sigma_0 - i \sin\alpha_j(x) \sigma_3$.
...
\section{Sample histograms and comparisons to old results}
...
\end{align}   
Histograms corresponding to these are shown in Fig~\ref{fig:A0_only}.
\begin{figure}[H]
	\centering
...
\paragraph{Warning.} The following can contain errors in the details. If you're reading this in 2024 or later, see my recent papers instead.
...
& + \frac34 g^4 \Big( \frac{15}{16}\Sigma^2 + \frac{\pi}{3} \Sigma + \frac54 - \frac72 \delta - 4\rho +4\kappa_1 - \kappa_2 - \kappa_3 - 3\kappa_4  \Big) \Big].
\end{align}
These were taken from \cite{Kajantie:1996qd}, with some guessing based on similarity of $\gr{SU(2)}$ and $\gr{U(1)}$ Feynman diagrams. This was necessary because they only gave numerical values for terms on the last two rows. \lauri{(See our 2024 paper for exact expressions.)}
...
% 
& + 9 g^2 \lambda \left(\delta - \frac14 \Sigma^2 \right) - \frac34 g^4 \left( \frac{15}{16} \Sigma^2  + \frac{\pi}{3}\Sigma + \frac54 - \frac{11}{2}\delta - 6\rho + 4\kappa_1 - 2\kappa_4 \right)\Big]  .
\end{align}
