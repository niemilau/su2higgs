...
For the "easy" order parameter $\phi^\dagger\phi$, the acceptance rate of the multicanonical update is generally good, $>90\%$. This is in agreement with Kari's acceptance rates in the SM and MSSM.
...
This is exactly the same action as in Ref.~\cite{Kajantie:1997tt}, but they denote $\Sigma = A_0, m_\Sigma = m_D, b_4 = \lambda_A$. They define 
\begin{align}
y = \frac{m^2_D(g^2)}{g^4}, \quad x = \frac{\lambda_A}{g^2},
\end{align}
in terms of which they give their results. In Fig.~4 they show histograms but only give the critical $y_c$ for the case $x=0.20$, in Fig.~2. Their volume is also quite large, $28^3$, and we need multicanonical in order to simulate this volume. Nevertheless, reading off the critical $y$ corresponding to this volume and $\beta_G = 16$ from their Fig.~2, we have $y_c \approx 0.154$. This fixes the continuum mass $\mu^2_\Sigma$ in our parameterization. 
...
\begin{figure}[h]
	\includegraphics[scale=0.5]{histogram_SM}
	\caption{Histogram for the SM case with inputs as in Eq.~(\ref{eq:SM-inputs}), at volume $6^2 \times 18$. The separation of the peaks agrees well with Kari's histogram in Fig.~6 when the conversion of Eq~(\ref{eq:kari_fields}) is taken into account. For larger volumes, this point would be a good place to test the multicanonical update for the doublet.}
\end{figure}
...
The resulting histogram from David's code is shown in Fig.~\ref{fig:histogram_SM2} and is in perfect agreement with what they have, even without multicanonical weighting. We conclude that the SM-only part of the code works very well. 

\begin{figure}[h]
\label{fig:histogram_SM2}
	\includegraphics[scale=0.5]{histogram_mhstar-70}
	\caption{Histogram for the SM case, $m_H^* = 70$ GeV, with inputs as in Eq.~(\ref{eq:SM-inputs2}), at volume $24^3$. The result is in very good agreement with Kari's histogram.}
\end{figure}
...
We can compare the full fundamental + adjoint case to the histograms in Fig.~23 of Ref.~\cite{Kajantie:1995kf}. In this case $\Sigma^a$ is identified as the temporal scalar $A_0^a$ originating from thermal screening of the gauge field $A^a_\mu$. Our DR suggests that the parameters need to be fixed (in terms of 4d parameters) as 
\begin{align}
