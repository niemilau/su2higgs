...
\end{align}
where $\sigma_3$ is the third Pauli matrix (to check that this is the correct transformation rule, apply $\phi \rightarrow e^{i\alpha} G(x) \phi$ on the vector $\phi$ and see how the elements of $\Phi$ transform). The plaquette trace for $\gr{U(1)}$ can be written as 
\begin{align}
