...
int main(int argc, char *argv[]) {
  ...
  layout(&p, &comlist);
	...
	if (p.multicanonical) {
		...
		load_weight(p, &w);
		// initial parameter redefinitions due to the weight
		double muca_param = calc_orderparam(p, f);
		long bin = whichbin(w, muca_param);
		muca_param_shift(&p, w, bin, 0.0);
	...
	while (iter <= p.iterations) {
		...
		if ((iter % p.checkpoint == 0) && (iter != start)) {
			...
			if (!p.rank) {
				...
				#endif
				if (p.multicanonical) {
					printf0(p, "multicanonical %lu/%lu (%.2lf%%), ", c.accepted_muca, c.total_muca,
