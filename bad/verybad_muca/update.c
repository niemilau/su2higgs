	...
	for (long i=0; i<p->sites; i++) {
		if (p->parity[i] == parity) {
			if (p->algorithm_su2doublet == OVERRELAX && (metro != 1)) {
				...
				// do normal canonical update WITHOUT caring about the weight?
				// TODO remove weight argument if it really is not needed

				c->accepted_doublet += metro_doublet(f, *p, i, w);
				c->total_doublet++;
			}
		}
	}
...
/* Same as checkerboard_sweep_su2doublet(), but performs a transverse Metropolis update
* that leaves phisq invariant. Useful for improving ergodicity in multicanonical runs
* using phisq as the order parameter.
*/
void transverse_sweep_doublet(fields f, params p, counters* c, char parity) {
	for (long i=0; i<p.sites; i++) {
		if (p.parity[i] == parity) {
			metro_doublet_transverse(f, p, i);
		}
	}
}
