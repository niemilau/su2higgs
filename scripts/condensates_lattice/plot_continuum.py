...
plt.errorbar(data1[:,0], 2*h1cont/T, yerr=2*data1[:,3], fmt='o', label = r"$(\phi^\dagger_1 \phi_1)_\text{3d}$",markersize=10)
plt.errorbar(data2[:,0], 2*h2cont/T, yerr=2*data2[:,3], fmt='o', label = r"$(\phi^\dagger_2 \phi_2)_\text{3d}$",markersize=10)


# draw horizontal line at y = 0
plt.axhline(y=0, color='black', linestyle='-')

#plt.title(r'Mikko BM1: $m_H = 66$ GeV, $m_{A} = m_{H^\pm} = 300$ GeV, 4d RG scale $= 7T$, 3d RG scale $= T$ \newline ($V=48^3$, $\beta_g = 16$). Note: fields are the 3d fields',fontsize=12)
plt.xlabel(r'$T$ / GeV',fontsize=26)
h = plt.ylabel(r'$2\langle\phi^\dagger\phi\rangle_\text{3d}/T',fontsize=26)
plt.grid()
plt.tight_layout()
plt.tick_params(labelsize=26)
plt.legend(loc='lower right',prop={'size': 20},framealpha=1)
#h.set_rotation(0)
#plt.xlim([0,0.2])
plt.ylim([-0.8,2.5])
#plt.xticks(np.arange(min(T), max(T)+1, 1.0)) # set denser ticks
pyplot.legend();
pyplot.savefig('BM1_beta16_condensate.pdf')
plt.gcf().clear()
